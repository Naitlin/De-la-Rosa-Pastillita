...
$('#boton-ir-general').click(() => {
    cambiarEstadoElemento(manual);
    cambiarEstadoElemento(generalContent);
});

$('#boton-ir-especifico').click(() => {
    cambiarEstadoElemento(manual);
    ...
    enfermedades.forEach(enfermedad => {
        ...
        <label class="">
            ...
            <div class="card card-body d-flex flex-row justify-content-between align-items-center">
                <div>${ enfermedad.nombre } <a class="icon_info_enfermedad" title="Más información..." target="_blank" href="informacion.html?id=${ enfermedad.id }">ⓘ</a></div>
            </div>
        </label>
        `;
        h += item;
...
const cambiarEstadoElemento = elm => { elm.toggleClass("elm-oculto");$("html, body").animate({ scrollTop: 0 }, "slow");}
...
const evaluacionGeneral = () => {
    ...
    resultados.forEach(res => {
        ...
        hSintomas = `
        <li>
            <a href="informacion.html?id=${ res.enfermedad.id }" target="_blank" rel="noopener noreferrer">${ res.enfermedad.nombre }</a>
        </li>
        `;
    });
    ...
    if (resultados.length == 1) {
        modalResultados.html(`
            ...
            <br>
            <small>
                Esta aplicación es para fines educativos. Contacte a su profesión de salud de preferencia si sospecha poseer un padecimiento.
            </small>
        `);
...
const evaluacionEspecifica = () => {
    ...
    if(evChkbx.length < 2) {
        ...
        $('html,body').animate({
            scrollTop: $('#alert-minimo').offset().top
          },'slow');
    }
    ...
    evChkbx.forEach(chk => {
        hEnfermedades += `
            <li>
            <a href="informacion.html?id=${ chk.id }" target="_blank" rel="noopener noreferrer"> ${ chk.nombre } </a>
            </li>
        `;
    });
...
const actualizarSlider = idSintoma => {
    ...
    if (valslider == 0) small.innerText = `${ valores[0] }`;
    if (valslider == 10) small.innerText = `${ valores[1] }`;
    if (valslider == 20) small.innerText = `${ valores[2] }`;
    if (valslider == 30) small.innerText = `${ valores[3] }`;
    if (valslider == 40) small.innerText = `${ valores[4] }`;
    if (valslider == 50) small.innerText = `${ valores[5] }`;
    if (valslider == 60) small.innerText = `${ valores[6] }`;
    if (valslider == 70) small.innerText = `${ valores[7] }`;
    if (valslider == 80) small.innerText = `${ valores[8] }`;
